...
AoATOC.html - Plantation Productions, Inc
www.plantation-productions.com
The Published Edition is now Available! The Art of Assembly Language is now available from No Starch Press (http://www.nostarch.com) The professionally edited edition ...
...
void mmx_brighten(BITMAP* bitmap, INT brighten, BYTE* buffer) {
	...
	__asm
	{
		...
		//mm2 storing brigthen values to all single bytes
		xor edi, edi;
		pxor mm2, mm2;
		pxor mm1, mm1;
	...
	loop_begin1 :
		...
		je loop_end1;
		jmp loop_begin1;
		//mm2 contains the brigten values in each byte interval
	loop_end1:
...
void assembly_brighten(BITMAP* bitmap, INT brighten, BYTE* buffer) {
	...
	__asm
	{
		...
		mov edi, eax //edi is the bytes of total pixel 
		xor ecx, ecx
		//create a loop
		start :
		//loop while still have pixels to brighten
		cmp ecx, edi
			je end //may have to use different jmp condition
				   //jump out of loop if done

				   //load a pixel into a register

				   //free reisters: eax, ebx, edx
			mov edx, [esi + ecx]
			shr edx, 24
			and edx, 0x000000ff
			...
			//test if birgthen value plus values in register is over 255
			//values are in al ah bl
...
LRESULT CALLBACK HelloWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_LBUTTONDOWN:
		...
		QueryPerformanceCounter((LARGE_INTEGER*)&ends);
		results = ends - starts;
		sprintf_s(msg, "time it took:%d", results);
	...
	case WM_RBUTTONDOWN:
		...
		QueryPerformanceCounter((LARGE_INTEGER*)&starts2);
		//assembly_brighten(&Bitmap, 30, temppBits);
		nonAsMbrighten(&Bitmap, 30, temppBits);
		QueryPerformanceCounter((LARGE_INTEGER*)&ends2);
		results2 = ends2 - starts2;
		sprintf_s(msg, "time it took:%d", results2);
		SetWindowText(hwnd, msg);
		SelectObject(hdcMem, hBitmap);
